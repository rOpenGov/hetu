...
The **hetu R package** provides tools to work with Finnish personal identity numbers (hetu). Some functions can also be used with Finnish Business ID numbers (y-tunnus).
...
The package has also the ability to generate Finnish Business ID numbers (y-tunnus) and check their validity. Unlike with personal identification numbers, no additional information can be extracted from Business IDs.
...
A similar function exists for Finnish Business IDs (y-tunnus). 

```{r hetu_ctrl_example2, fig.message=FALSE}
...
Information contained in the PIN can be extracted with a generic extract parameter. Valid values for extraction are *hetu*, *sex*, *personal.number*, *checksum*, *date*, *day*, *month*, *year*, *century*, *valid.pin* and *is.temp*. 

*is.temp* can be extracted only if allow.temp is set to TRUE, otherwise temporary PINs are filtered from the output.
...
## Generating random PINs and BIDs
...
By using extract, the user can limit the printed columns to only those which have FALSE values.
...
When subsetting is set to TRUE, the function prints only rows with FALSE value in the extracted test.
...
Because of the way PINs are handled in different functions, the diagnostics-function can show unexpected warning messages or introduce NAs by coercion if the date-part of the PIN is too long. This may result in inability to handle the PIN at all!
